...
real simulate_fo_fixed_copy_to_gpu(sim_data* sim, particle_simd_fo *p_ptr, particle_simd_fo *p0_ptr, B_field_data* Bdata, E_field_data* Edata, particle_loc*  p_loc, real* hin, real* rnd, int* ps, int* sort_index);
...
#ifdef GPU
    simulate_fo_fixed_copy_to_gpu(sim, p_ptr, p0_ptr, Bdata, Edata, &p_loc, hin, rnd, ps, sort_index);
#endif    
    while(n_running > 0) {
        /* Store marker states */
        //#pragma omp simd
